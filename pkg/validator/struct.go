...
const (
	...
	AlphabetLower = "alfabet lower case"
	AlphabetUpper = "alfabet upper case"
	Numeric       = "numeric"
	Dash          = "dash"
)
...
var errorList = map[string]string{
	customTagRegexp: fmt.Sprintf("The parameter %s must be =", validateValue),
}
...
func (v *StructValidator) setTranslationOverride(tag string, message string) {
	...
	if err != nil {
		log.Println(logger.StringYellow(fmt.Sprintf("Struct Validator: warning, failed set translation validator on tag [%s]", tag)))
	}
}
